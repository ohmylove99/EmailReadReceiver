...
namespace Octopus.Email.Excel
{
    public class ExcelProcess
    {
        public DataTable Run(string fileName)
        {
            using (ExcelHelper helper = new ExcelHelper(fileName))
            {
                DataTable dataTable = helper.ExcelToDataTable("",false);
                return dataTable;
            }
        }

        public DataTable Run(string fileName, int pHojaIndex)
        {
            DataTable dataTable = RunTest(fileName, 0);
            return dataTable;
        }

    
        private DataTable RunTest(string pRutaArchivo, int pHojaIndex)
        {
